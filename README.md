...
| 💌 情侣点菜 | 从菜谱详情加入点菜篮，生成一份 30 天有效的菜单并转发给另一半查看 |
...
5. **创建分享菜单集合**：云数据库中新建集合 `shared_menus`。建议关闭客户端直接读写权限；分享菜单只通过 `recipes` 云函数访问。
6. 重新编译，即可从云数据库拉取（详情页/列表/随机和情侣菜单分享走云，搜索仍走内置索引以保证食材反查与离线可用）。

> 情侣点菜的分享记录需要跨设备读取，因此该功能只在配置并部署云开发后可用；本地数据模式仍可正常使用其余功能。
